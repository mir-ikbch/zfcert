...
ただし、名前を明示する`specialize`、`apply`、`exact`、`obtain`、`cases`、`rewrite`
などのタクティクからは参照できます。必要な事実を`resolution.`の探索対象にも
加える場合だけ、`put`で現在の仮定へ明示的に追加します。
...
名前を指定する通常のタクティクからは参照できます。`resolution.`にも使わせる
定理だけを`put`で現在の仮定へ明示的に追加します。
